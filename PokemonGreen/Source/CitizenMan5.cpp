...
namespace game_framework
{
		...
		Citizen()
	{
		...
		normalDialog.AddText("you can press enter to;open the menu!");
	}

	void CitizenMan5::KeyDownListener(UINT nChar, CHero &hero)
	{
	...
	void CitizenMan5::LoadBitmap()
	{
		normalImg[up].LoadBitmap(IDB_MAN5_UP, RGB(255, 0, 0));
		normalImg[down].LoadBitmap(IDB_MAN5_DOWN, RGB(255, 0, 0));
		normalImg[left].LoadBitmap(IDB_MAN5_LEFT, RGB(255, 0, 0));
		normalImg[right].LoadBitmap(IDB_MAN5_RIGHT, RGB(255, 0, 0));
	}

	void CitizenMan5::OnShow(CHero &hero)
	{
	...
	void CitizenMan5::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void CitizenMan5::OnMove(CHero &hero) // for don't move
	{
		if (!isEvent) {
	...
	void CitizenMan5::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void CitizenMan5::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
