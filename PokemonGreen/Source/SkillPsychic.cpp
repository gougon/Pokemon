...
namespace game_framework {
	SkillPsychic::SkillPsychic()
	{
		attribute = psychic;
	}

	int SkillPsychic::Damage(Pokemon *self, Pokemon *enemy)
	{
	...
	AttrDamage SkillPsychic::DamageEffect(Pokemon *enemy)
	{
		if (enemy->GetAttribute() == physical ||
			enemy->GetAttribute() == steel
			) {
			return lowEffe;
		}
		else if (enemy->GetAttribute() == poison ||
			enemy->GetAttribute() == fighting
			) {
			return superEffe;
		}
		else {
