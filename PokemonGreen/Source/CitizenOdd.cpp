...
namespace game_framework
{
		...
		Citizen()
	{
		...
		normalDialog.AddText("thank you for save my live!");
		normalDialog.AddText("can you let my daughter may back");
	}

	void CitizenOdd::KeyDownListener(UINT nChar, CHero &hero)
	{
	...
	void CitizenOdd::LoadBitmap()
	{
		normalImg[up].LoadBitmap(IDB_ODD_UP, RGB(255, 0, 0));
		normalImg[down].LoadBitmap(IDB_ODD_DOWN, RGB(255, 0, 0));
		normalImg[left].LoadBitmap(IDB_ODD_LEFT, RGB(255, 0, 0));
		normalImg[right].LoadBitmap(IDB_ODD_RIGHT, RGB(255, 0, 0));
	}

	void CitizenOdd::OnShow(CHero &hero)
	{
	...
	void CitizenOdd::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void CitizenOdd::OnMove(CHero &hero) // for don't move
	{
		if (!isEvent) {
	...
	void CitizenOdd::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void CitizenOdd::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
