...
{
	TrainerGymYamao::TrainerGymYamao(AtkInterface *atkInterface) :
		Trainer(atkInterface)
	{
		...
		// set pokemons
		PokemonFactory pmf;
		pokemons.push_back(pmf.CreateEnemy(metang, 12));
		pokemons.push_back(pmf.CreateEnemy(smoochum, 13));
	...
	void TrainerGymYamao::KeyDownListener(UINT nChar, CHero &hero)
	{
		...
		case KEY_Z:
			if (!isEvent) {
				...
				if (eventDialog.IsEnd()) {
					StopTalk(hero);
					hero.SetGameEvent(winYamao, true);
				}
			}
		}
	}
	...
	void TrainerGymYamao::OnShow(CHero &hero)
	{
	...
	void TrainerGymYamao::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void TrainerGymYamao::OnMove(CHero &hero)
	{
	...
	void TrainerGymYamao::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void TrainerGymYamao::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
