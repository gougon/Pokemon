...
void CMovingBitmap::LoadBitmap(int IDB_BITMAP, COLORREF color)
{
    ...
    rectWidth = location.right;
    rectHeight = location.bottom;
...
void CMovingBitmap::SetWidth(int width)
{
    // bitmapSize.bmWidth = width;
    // CDDraw::BitmapRect[SurfaceID].right = bitmapSize.bmWidth;
    rectWidth = (width <= 0) ? 1 : width;
}

void CMovingBitmap::SetHeight(int height)
{
    // bitmapSize.bmWidth = width;
    // CDDraw::BitmapRect[SurfaceID].right = bitmapSize.bmWidth;
    rectHeight = (height <= 0) ? 1 : height;
}
...
void CMovingBitmap::ShowBitmap(string hpBar)
{
    GAME_ASSERT(isBitmapLoaded, "A bitmap must be loaded before ShowBitmap() is called !!!");
    CDDraw::BltBitmapToBack(SurfaceID, location.left, location.top, rectHeight, rectWidth);
}
...
int CMovingBitmap::RectWidth()
{
    return rectWidth;
}

int CMovingBitmap::RectHeight()
{
    return rectHeight;
}
...
void CDDraw::BltBitmapToBack(unsigned SurfaceID, int x, int y, int rectHeight, int rectWidth)
{
    GAME_ASSERT(lpDDSBack && (SurfaceID < lpDDS.size()) && lpDDS[SurfaceID], "Internal Error: Incorrect SurfaceID in BltBitmapToBack");
    CRect TargetRect;
    TargetRect.left = x;
    TargetRect.top = y;
    TargetRect.right = x + (int)((rectWidth - BitmapRect[SurfaceID].left));
    TargetRect.bottom = y + (int)((rectHeight - BitmapRect[SurfaceID].top));
    int blt_flag;
