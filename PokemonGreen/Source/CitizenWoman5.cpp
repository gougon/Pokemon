...
namespace game_framework
{
		...
		Citizen()
	{
		...
		normalDialog.AddText("get the pokeball! get the pokemon!!");
	}

	void CitizenWoman5::KeyDownListener(UINT nChar, CHero &hero)
	{
	...
	void CitizenWoman5::LoadBitmap()
	{
		normalImg[up].LoadBitmap(IDB_WOMAN5_UP, RGB(255, 0, 0));
		normalImg[down].LoadBitmap(IDB_WOMAN5_DOWN, RGB(255, 0, 0));
		normalImg[left].LoadBitmap(IDB_WOMAN5_LEFT, RGB(255, 0, 0));
		normalImg[right].LoadBitmap(IDB_WOMAN5_RIGHT, RGB(255, 0, 0));
	}

	void CitizenWoman5::OnShow(CHero &hero)
	{
	...
	void CitizenWoman5::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void CitizenWoman5::OnMove(CHero &hero) // for don't move
	{
		if (!isEvent) {
	...
	void CitizenWoman5::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void CitizenWoman5::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
