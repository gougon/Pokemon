...
namespace game_framework
{
	Bag::Bag() :
		...
		yesnoChooser(false), successReceive(false), isSelectball(false), pk_Menu(nullptr)
	{
		// empty body
	}
	...
	void Bag::OnShow()
	{
		if (pk_Menu != nullptr && pk_Menu->IsWork() && dynamic_cast<PokemonMenu*>(pk_Menu)->IsItemReceive()) pk_Menu->OnShow();
		else
		{
	...
	void Bag::OnMove()
	{
		if (pk_Menu != nullptr && pk_Menu->IsWork() && dynamic_cast<PokemonMenu*>(pk_Menu)->IsItemReceive())  pk_Menu->OnMove();
		else
		{
			cursor.OnMove();
			if (inSellMode) {
			...
			if (pk_Menu != nullptr && (dynamic_cast<PokemonMenu*>(pk_Menu)->SuccessToUseItem() || dynamic_cast<PokemonMenu*>(pk_Menu)->SuccessToTakeItem())) {
	...
	void Bag::KeyDownListener(UINT nChar)
	{
		...
		if (pk_Menu != nullptr && pk_Menu->IsWork() && dynamic_cast<PokemonMenu*>(pk_Menu)->IsItemReceive())
		...
		else
		{
			if (inSellMode) {
				if (inShowText) {
					...
					if (nChar == KEY_UP || nChar == KEY_DOWN || nChar == KEY_Z || nChar == KEY_X)
						CAudio::Instance()->Play(AUDIO_SELECT);
					switch (nChar) {
					case KEY_UP: 
						yesnoChooser = true; 
				...
				if (inYesno) {
					...
					if (nChar == KEY_UP || nChar == KEY_DOWN || nChar == KEY_LEFT || nChar == KEY_RIGHT || nChar == KEY_Z || nChar == KEY_X)
						CAudio::Instance()->Play(AUDIO_SELECT);
					switch (nChar) {
					case KEY_UP:
	...
	void Bag::DropItem(int itemId, int amount)
	{
		for (int i = 0; i < (int)items.size(); ++i) {
			if (items.at(i)->GetID() == itemId) {
				if (item_amount[itemId] == amount) 
					item_amount[itemId] = 0;
				else 
					item_amount[itemId] -= amount;
				delete items.at(i);
