...
namespace game_framework
{
		...
		Citizen()
	{
		...
		normalDialog.AddText("i want a pokemon;can you give me?");
	}

	void CitizenBoy::KeyDownListener(UINT nChar, CHero &hero)
	{
	...
	void CitizenBoy::LoadBitmap()
	{
		normalImg[up].LoadBitmap(IDB_BOY_UP, RGB(255, 0, 0));
		normalImg[down].LoadBitmap(IDB_BOY_DOWN, RGB(255, 0, 0));
		normalImg[left].LoadBitmap(IDB_BOY_LEFT, RGB(255, 0, 0));
		normalImg[right].LoadBitmap(IDB_BOY_RIGHT, RGB(255, 0, 0));
	}

	void CitizenBoy::OnShow(CHero &hero)
	{
	...
	void CitizenBoy::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void CitizenBoy::OnMove(CHero &hero) // for don't move
	{
		if (!isEvent) {
	...
	void CitizenBoy::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void CitizenBoy::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
