...
namespace game_framework
{
		...
		Citizen()
	{
		...
		normalDialog.AddText("i work for professor odd");
		normalDialog.AddText("but someday he disappear...");
	}

	void CitizenMan1::KeyDownListener(UINT nChar, CHero &hero)
	{
	...
	void CitizenMan1::LoadBitmap()
	{
		normalImg[up].LoadBitmap(IDB_MAN1_UP, RGB(255, 0, 0));
		normalImg[down].LoadBitmap(IDB_MAN1_DOWN, RGB(255, 0, 0));
		normalImg[left].LoadBitmap(IDB_MAN1_LEFT, RGB(255, 0, 0));
		normalImg[right].LoadBitmap(IDB_MAN1_RIGHT, RGB(255, 0, 0));
	}

	void CitizenMan1::OnShow(CHero &hero)
	{
	...
	void CitizenMan1::ReShow(CHero &hero)
	{
		int hx = hero.GetX1() / SM;
	...
	void CitizenMan1::OnMove(CHero &hero) // for don't move
	{
		if (!isEvent) {
	...
	void CitizenMan1::Talk(CHero &hero)
	{
		isTalk = true;
	...
	void CitizenMan1::SetTopLeft(int hx, int hy)
	{
		int dx = x * SM - hx;
		int dy = y * SM - hy;
